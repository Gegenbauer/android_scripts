...
class EnvSetup:
    ...
    def get_path(self, path_type: PathType, project_name: str = project_name) -> str:
        """
        ...
        appdirs_func = self._appdirs_map[path_type]
        path = appdirs_func(project_name, appauthor=False)

        from script_base.log import logger
        logger.debug(f"{env_key} not found in environment variable, using appdirs default path: {path}")
        return path
    ...
    def get(self, path_type: PathType, project_name: str = project_name) -> str:
        """
